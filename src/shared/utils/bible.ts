...
const BOOK_ABBREVIATIONS: Record<string, string> = {
  ...
  // Позволяет getFullBookName/normalizeBookNameForUrl работать с уже полными именами
  'Бытие': 'Бытие',
  'Исход': 'Исход',
  'Левит': 'Левит',
  'Числа': 'Числа',
...
/**
 * Normalizes a book name to its canonical form for use in URL paths.
 * Handles abbreviations, alternate spellings, and full "Евангелие от X" forms.
 */
export function normalizeBookNameForUrl(bookName: string): string {
  if (!bookName) return bookName;
  return getFullBookName(bookName);
}
