// @vitest-environment jsdom
import { describe, expect, it, vi } from 'vitest';
import { fireEvent, render, screen } from '@testing-library/react';
import { SongKeyPicker } from './SongKeyPicker';

const OPTIONS = ['C', 'C#', 'D', 'Eb', 'E', 'F', 'F#', 'G', 'Ab', 'A', 'Bb', 'B'];

function renderPicker(props: Partial<React.ComponentProps<typeof SongKeyPicker>> = {}) {
  const onChange = vi.fn();
  const onReset = vi.fn();
  const utils = render(
    <SongKeyPicker value="G" source="original" options={OPTIONS} onChange={onChange} onReset={onReset} {...props} />,
  );
  return { ...utils, onChange, onReset };
}

describe('SongKeyPicker', () => {
  it('показывает подпись источника для личной и основной тональности', () => {
    const personal = renderPicker({ value: 'Ab', source: 'personal' });
    expect(personal.container.querySelector('[data-song-key-picker-value]')?.textContent).toBe('Ab');
    expect(personal.container.querySelector('[data-song-key-picker-toggle]')?.textContent).toContain('моя');
    personal.unmount();

    const byDefault = renderPicker({ value: 'A', source: 'default' });
    expect(byDefault.container.querySelector('[data-song-key-picker-toggle]')?.textContent).toContain('по умолчанию');
  });

  it('для исходной тональности подписи источника нет', () => {
    const { container } = renderPicker({ value: 'G', source: 'original' });
    expect(container.querySelector('[data-song-key-picker-toggle]')?.textContent).toBe('G');
  });

  it('«сбросить» видна только при личной тональности', () => {
    const original = renderPicker({ source: 'original' });
    expect(original.container.querySelector('[data-song-key-picker-reset]')).toBeNull();
    original.unmount();

    const personal = renderPicker({ source: 'personal' });
    expect(personal.container.querySelector('[data-song-key-picker-reset]')).not.toBeNull();
    fireEvent.click(personal.container.querySelector('[data-song-key-picker-reset]') as HTMLElement);
    expect(personal.onReset).toHaveBeenCalledTimes(1);
  });

  it('список тональностей раскрывается по кнопке и закрывается после выбора', () => {
    const { container, onChange } = renderPicker();
    expect(container.querySelector('[data-song-key-picker-options]')).toBeNull();

    fireEvent.click(container.querySelector('[data-song-key-picker-toggle]') as HTMLElement);
    expect(container.querySelector('[data-song-key-picker-options]')).not.toBeNull();

    fireEvent.click(screen.getByRole('button', { name: 'Ab' }));
    expect(onChange).toHaveBeenCalledWith('Ab');
    expect(container.querySelector('[data-song-key-picker-options]')).toBeNull();
  });

  it('действующая тональность отмечена выбранной в списке', () => {
    const { container } = renderPicker({ value: 'Eb' });
    fireEvent.click(container.querySelector('[data-song-key-picker-toggle]') as HTMLElement);
    expect(screen.getByRole('button', { name: 'Eb' }).getAttribute('aria-pressed')).toBe('true');
    expect(screen.getByRole('button', { name: 'G' }).getAttribute('aria-pressed')).toBe('false');
  });
});
