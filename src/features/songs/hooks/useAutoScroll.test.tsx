...
/**
 * Гистерезис `canScroll` (фокус-режим автоскролла): пока идёт проигрывание, замер
 * подтверждает флаг, но не снимает его. Причина — свёрнутый хром увеличивает
 * `clientHeight` контейнера, и на песне «ровно на высоту шапки» честный замер убрал бы
 * FAB из DOM, что вернуло бы шапку и FAB (мигание).
 */
...
describe('useAutoScroll — гистерезис canScroll', () => {
  it('во время проигрывания выросший clientHeight не сбрасывает canScroll', () => {
    ...
    // Свёрнутый хром: вьюпорт вырос ровно до высоты контента.
