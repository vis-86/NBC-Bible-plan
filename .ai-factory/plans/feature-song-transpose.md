...
## Решения Игоря от 2026-07-25 (закрывают NEEDS DECISION)

- Селектор при `showChords: false` — **прятать** (аккордов на экране нет, настройка ни на что
  не влияет).
- Названия тональностей — **латиницей** (`G`, `Em`, `Ab`), не русскими названиями из текста §10.1:
  так же, как в корпусе `{key:}` и в аккордах на листе, без таблицы перевода и расхождений.
  Подписи источника: `G` (исходная, без подписи) / `A (по умолчанию)` / `Ab (моя)`.
- Действующая тональность показывается в шапке вместо плашки с исходной; темп остаётся плашкой.
...
- [x] **Task 1 — Классификация маркеров**
...
- [x] **Task 2 — Зависимость `tonal` + транспозиция аккорда**
...
- [x] **Task 3 — Разрешение действующей тональности**
...
- [x] **Task 5 — Персист личной тональности**
...
- [x] **Task 6 — Селектор тональности в шапке песни** (depends on 3, 5)
...
- [x] **Task 7 — Применение транспозиции в рендере** (depends on 2, 6)
...
- [x] **Task 8 — Регрессия на экстремумах корпуса** (depends on 1–7)
...
## Итог этапа (2026-07-25)

Все 8 задач выполнены. Отклонения от плана — три, все в сторону сужения:

1. **Регрессия корпуса читает `data/songs/*.chordpro` напрямую** (`transpose.corpus.test.ts`),
   а не копии в `__fixtures__`: корпус лежит в репозитории и именно он импортируется в Directus,
   копия шести песен разошлась бы при первом переимпорте. Покрыт весь корпус (97 песен), а не
   только экстремумы §12.
2. **`+12` полутонов проверяется по высоте (chroma), а не по строке**: спеллинг берётся из
   целевой тональности, поэтому не-диатонический `Eb` в тональности G легально возвращается
   как `D#` (§10.2). Строковое равенство здесь означало бы ошибку в требовании, а не в коде.
3. **Личная тональность читается через `useSyncExternalStore`**, а не `useState` + эффект:
   ESLint-правило `react-hooks/set-state-in-effect` (React Compiler) запрещает синхронный
   `setState` в эффекте. Из-за этого в `personalKeyStore` добавлена `subscribePersonalKeys`.

Сверх плана: e2e-тест «после смены тональности листы пересобираются, пустых нет»
(`e2e/layout/song-layout.spec.ts`) — Task 7 тронул `layoutSignature` в `useSheets`, а этот
класс дефектов виден только в реальном браузере.

Числа: **493 теста** (baseline 421), lint **59 errors / 30 warnings** (baseline не превышен,
в тронутых файлах ошибок нет), `npx tsc --noEmit` чисто, `yarn playwright test e2e/layout`
**10/10** на прод-контуре (build + bff + static:serve).

Ручная проверка в браузере (dev-контур): песня 1 «Аллилуйя» G→Bb — `C/G` → `Eb/Bb`, бемоли из
Bb-мажора, `[|]` не изменились; персист держится после перезагрузки; песня 37 «Придите и
прославьте все» A→Eb — `[(пауза)]` не изменилась, число маркеров сохранено; сброс возвращает
исходную тональность и убирает кнопку; в режиме «только текст» селектор не рендерится,
в плашку возвращается `A · 60`.
