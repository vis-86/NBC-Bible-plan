...
- [x] **T13b. Drag-reorder (решение по NEEDS DECISION №2, пост-план).** Зависит от T13.
  - Решение: **Framer Motion `Reorder`** (`motion/react`, уже зависимость проекта — не
    `@dnd-kit`, чтобы не тащить новый пакет ради того, что уже есть во фреймворке).
    Референс подхода (dnd-kit, drag handle + keyboard sensor) смотрели в
    `nbc-music-chordpro/chordpro-app/src/pages/SetlistSidebar.tsx`, но библиотеку не
    переносили — тот же UX (ручка `⠿`/`GripVertical`, тянуть можно только за неё) собран
    на уже установленном `motion`.
  - `SetlistItemRow` — `Reorder.Item` с `dragListener={false}` + `useDragControls()`,
    старт драга только с ручки (`onPointerDown` на `GripVertical`), чтобы клики по
    кнопкам/тексту не перехватывались drag'ом.
  - `SetlistBuilder` — `reorderList` обёрнут в `Reorder.Group as="ul" axis="y"`,
    `onReorder` зовёт новый `reorderSongs(songIds)` в `useSetlistDraft`.
  - Кнопки «Вверх/Вниз» остаются — обязательная a11y-альтернатива (drag не заменяет,
    а дополняет), независимо работают через `moveSong`.
  **Логирование:** `console.debug('[SetlistBuilder] drag reorder ->', songIds)`.
  **Тесты:** `useSetlistDraft.test.ts` — `reorderSongs` заменяет порядок целиком.
  Drag pointer-events в jsdom не симулировался (нестабильно) — проверено вручную в
  браузере (drag за ручку + kнопки Вверх/Вниз вместе, консоль чистая).

- [x] **T13c. Переработка UX билдера (пост-план, по итогам показа Игорю).** Заменяет часть T13/T13b.
  - **Навигация:** полоса сетов убрана с дашборда (`SetlistDashboardStrip` удалён), вместо неё
    пункт «Сеты» (`ListMusic`) в `BottomNavBar` рядом с «Песнями». Пунктов становится 6 при
    включённом AI — ширина пункта переведена с `w-16` на `flex-1 max-w-16` (проверено: 375px и
    320px, переполнения нет, подписи в одну строку).
  - **Создание — два шага.** Шаг 1: поиск + галочки + лента чипов (тап = удалить) + сегмент
    `Все / Выбранные (N)`. Шаг 2 (`SetlistConfirmStep`, полноэкранный, НЕ BottomSheet — в шите
    нет места под drag-список): название, преднаполненное `Вск. Служение dd.mm.yyyy`, дата =
    ближайшее воскресенье (`setlistDefaults`), и порядок песен drag'ом. Шаг живёт в черновике
    (`draft.step`), чтобы переживать уход в песню и обратно.
  - **Правка существующего сета переехала из билдера в `SetlistView`:** кнопка «Добавить песню»
    (`AddSongsSheet` — поиск + галочки + кнопка «Добавить»), удаление строки, drag-порядок на
    месте. Каждая правка — оптимистичный PATCH с откатом состава при ошибке.
    `/dashboard/setlist-edit?id=` больше не поддерживается; путь маршрута сохранён (он в
    `APP_SHELL_ROUTES` и в прекеше SW установленных PWA).
  - **Drag почему не работал:** (1) на мобильном reorder-список рендерился только при
    `editingId`, т.е. при создании его на экране не было вовсе; (2) `dragListener={false}` +
    `dragControls.start` на кнопке-ручке — кнопка перехватывала pointer-цепочку, а
    `touch-action: none` стоял только на ручке, так что скролл забирал жест до старта drag'а.
    Решение: тянем строку целиком, `touch-none` на самом `Reorder.Item`, `GripVertical` —
    только визуальный аффорданс.
  - **Отдельный баг, пойманный в браузере:** Framer гасит click после drag только на самом
    draggable-элементе; обработчик открытия песни висит на ВЛОЖЕННОЙ кнопке названия, поэтому
    перетаскивание в `SetlistView` заканчивалось переходом на страницу песни. Лечится флагом
    `draggedRef` (ставится в `onDragStart`, снимается в `onPointerDown`).
  **Логирование:** `[SetlistBuilder] step -> …`, `[useSaveSetlist] create/update`,
  `[SetlistView] PATCH состава не прошёл, откатываем порядок`.
  **Проверено в браузере (375px):** drag меняет порядок и в DOM, и в черновике; PATCH уходит с
  верным `songIds`; тап по строке по-прежнему открывает песню; консоль чистая.
...
2. ~~**Drag-reorder как зависимость.**~~ **Решено (T13b/T13c):** Framer Motion `Reorder`
   (уже зависимость проекта), тянем строку целиком.
   **Осознанное a11y-исключение (решение Игоря, T13c):** кнопки «Вверх/Вниз» убраны
   полностью, клавиатурной альтернативы drag'у НЕТ — формально это WCAG 2.1 SC 2.1.1.
   Принято сознательно: аудитория — музыканты церкви на телефонах, указатель есть всегда.
   Если появится клавиатурный сценарий — возвращать через `ArrowUp/ArrowDown` на
   фокусируемой ручке, а не отдельными кнопками.
3. ~~**Вход в сетлисты для обычного читателя.**~~ **Решено (T13c):** пункт «Сеты» в
   `BottomNavBar` рядом с «Песнями»; полоса на дашборде убрана.
...
| 7 | T13b | `feat(setlists): drag-reorder in builder via Framer Motion Reorder` |
