...
- `song-annotations-offline.spec.ts` (T15, M10) — полный круг записи пометок:
  рисование БЕЗ сети (мышью по строке-якорю) → запись в `outbox` + стор `songState`
  → reload офлайн (пометки на месте, консоль чистая) → сеть вернулась → replay довёз
  запись в Directus → чистый IDB («другое устройство») читает пометки с сервера.
  Фикстур самоочищающийся: `afterAll` возвращает песне пустой набор тем же `PUT`,
  которым пишет приложение.

> **Рейт-лимит логина.** BFF пускает 10 логинов / 15 мин на IP (`server/src/routes/auth.ts`),
> а полный прогон тратит по одному на spec-файл. Повторные прогоны подряд упираются в
> лимит, и падение выглядит как таймаут `waitForURL('**/dashboard')` во ВСЕХ файлах
> сразу. Счётчик живёт в памяти процесса BFF — лечится его перезапуском.
