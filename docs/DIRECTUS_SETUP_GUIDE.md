...
Убедитесь, что Directus видит существующие таблицы в PostgreSQL (сервис `postgres`, база `directus`). Если коллекции не появились — создайте их через интерфейс; при ошибке подключения проверьте `DB_*`-переменные сервиса `directus` в `deploy/compose.yml`. Полная схема — в [Схеме БД](database-schema.md).
